...
pcl::PointIndices::Ptr sfnd::RANSAC<PointT>::findGroundPoints_()
{
    ...
    x1 = cloud_->at(best_i).x;
    x2 = cloud_->at(best_j).x;
    x3 = cloud_->at(best_k).x;

    y1 = cloud_->at(best_i).y;
    y2 = cloud_->at(best_j).y;
    y3 = cloud_->at(best_k).y;

    z2 = cloud_->at(best_i).z;
    z1 = cloud_->at(best_j).z;
    z3 = cloud_->at(best_k).z;
